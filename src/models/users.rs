...
impl Model {
    ...
    pub async fn has_active_user(db: &DatabaseConnection) -> ModelResult<bool> {
        let user = users::Entity::find()
            .filter(
                query::condition()
...
impl ActiveModel {
    ...
    pub async fn set_forgot_password_sent(mut self, db: &DatabaseConnection) -> ModelResult<Model> {
        ...
        // 生成 6 位数字作为重置密码的 token
        self.reset_token = ActiveValue::Set(Some(hash::random_string(6).to_uppercase()));
        Ok(self.update(db).await?)
    }
